<template lang="html">
<div class="register">
  <template v-if="isComplete">
    <h1 class="title">注册成功</h1>
    <form class="" method="post" @submit.prevent="onSubmit()">
      <ul>
        <li>请复制以下Token进行登录</li>
        <li><input type="text" name="" placeholder="手机号/邮箱" v-model.trim="token"></li>
      </ul>
      <button type="submit" name="button">{{registerState}}</button>
    </form>
  </template>

  <template v-else>
    <h1>欢迎加入豆瓣</h1>
    <form class="" method="post" @submit.prevent="onSubmit()">
      <ul>
        <li v-if="error">{{error}}</li>
        <li><input type="text" name="" placeholder="手机号/邮箱" v-model.trim="account"></li>
        <li class="pwd" v-if="pwdType==='password'">
          <input type="password" name="" placeholder="密码（最少6位）" v-model.trim="pwd">
          <span class="show_pwd"  :class="{show: isShow}" @click="showPwd()"></span>
        </li>
        <li class="pwd" v-if="pwdType==='text'">
          <input type="text" name="" placeholder="密码（最少6位）" v-model.trim="pwd">
          <span class="show_pwd"  :class="{show: isShow}" @click="showPwd()"></span>
        </li>
        <li><input type="text" name="" placeholder="昵称" v-model.trim="nickname"></li>
      </ul>
      <button type="submit" name="button" :disabled="isDisabled">{{registerState}}</button>
    </form>
    <p class="agree">点击「下一步」代表你已阅读并同意<a href="#">用户使用协议</a></p>
    <p class="downApp"><a href="#">下载豆瓣App</a></p>
  </template>
</div>
</template>

<script>

export default {
  data(){
    return {
      error:'',
      isComplete: false,
      registerState: '下一步',
      account:'',
      pwdType:'password',
      pwd:'',
      isShow:0,
      nickname:'',
      isDisabled:false
    }
  },
  methods:{
    showPwd(){
      this.isShow = !this.isShow,
      this.isShow ? this.pwdType = 'text' : this.pwdType = 'password'
    },
    beforeSubmit(){
      this.isDisabled = true,
      this.registerState = "正在提交..."
    },
    onSuccess(){
      this.isComplete = true
    },
    onError(){
      this.error = "抱歉，注册失败，请重新注册"
    },
    onSubmit(){
      this.beforeSubmit(),
      this.$store.dispatch({
        type:'register',
        account:this.account,
        pwd:this.pwd,
        nickname:this.nickname
      }).then(res =>{
        console.log(res)
      }).catch(err=>console.log(err))
    }
  }
}
</script>

<style lang="less" scoped>
.register {
  padding: 0 20px;
  h1{
    font-size: 40px;
    font-weight: 300;
    margin: 10% 0 9%;
    color: #42bd56;
    text-align: center;
  }
  form{
    width: 100%;
    ul{
      border: 1px solid #d8d8d8;
      border-radius: 6px;
      li{
        input{
          outline: none;
          width: 100%;
          box-sizing: border-box;
          padding-left: 10px;
          font-size: 14px;
          height: 45px;
          border-bottom: 1px solid #d8d8d8;
        }
        &:last-child{
          input{
            border-bottom: none
          }
        }
        &.pwd{
          position: relative;
          span{
            position: absolute;
            top: 0;
            right: 5px;
            height: 100%;
            width: 3.2rem;
            background-image: url(data:image/png;base64,);
            background-repeat: no-repeat;
            background-position: center center;
            background-size: 22px;
            z-index: 3;
            &.show {
              background-image: url(data:image/png;base64,);
            }
          }
        }
      }

    }
    button{
      border: none;
      outline: none;
      width: 100%;
      height: 45px;
      margin-top: 12px;
      font-size: 15px;
      color: #fff;
      background-color: #42bd56;
      border-radius: 4px;
    }
  }
  .agree{
    color: #d4d4d4;
    text-align: center;
    font-size: 14px;
    margin: 18px 0 40px;
    a{
      color: #d4d4d4;
      text-decoration: underline;
    }
  }
  .downApp {
    text-align: center;
    font-size: 14px;
    a{
      color: #42bd56;
    }
  }
}
</style>
